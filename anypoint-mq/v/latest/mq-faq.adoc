...
== Are Anypoint MQ objects such as Exchanges and Queues completely unique, and only accessible to the environment in which they were created? How do we ensure Exchanges or Queues created in one environment are not confused or connected to as Exchanges or Queues created in a different environment if they have the same name?

You can have the same object name for Queues and Exchanges between environments but the clientID/clientSecret are different. It's not possible to access a production queue from a development environment if they don't have the corresponding application client IDs.
