...
Create Mule 4 modules using the *Mule SDK*, the standard framework used to build components, connectors, message sources, and more, for use in Mule 4 applications.
...
[IMPORTANT]
*Anypoint Connector DevKit*-built connectors work out of the box with Mule 3.5.3 to Mule 3.9.x. Use *SDK* to build Anypoint Connectors for Mule 4.

== What is a Module?

A module is the Java code that is written and compiled to function in a Mule app. The module defines a contract between Mule and a resource, for example a cloud service or API, enabling ease of access to the service for the Mule app/integration developer, for example by providing the user the settings required for connection to the service.

The developer using Mule 4 SDK to code the module must provide the necessary parameters for all desired module functionality in specific classes. The developer must annotate the classes according to the predefined SDK model.
...
Use the `@Extension`, `@Configurations`, and `@Configuration` annotation to declare your extension and its configurations.
...
[NOTE]
`@Extension` marks a discoverable entry point for discovery and validation for use with Mule. Learn more about link/configurations[ways to create configurations].
