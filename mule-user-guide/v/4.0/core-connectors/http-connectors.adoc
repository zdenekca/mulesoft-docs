...
HTTP support for Mule includes two connectors, which are the *HTTP listener*, *HTTP requester*, and a *load static resource* operation. All are accessible from your Mule application design palette, typing "HTTP". The connectors require reference to a *connector configuration* global element. Define attributes there for reuse across HTTP connectors in your Mule application.

=== How to Use an HTTP Listener

Listen for requests that arrive at a certain address by placing the HTTP connector in the source (left) side of a Mule flow. Get back the generated HTTP response.
...
=== How to Load a Static HTTP Resource
...
== Migrating from Mule 3 HTTP Connectors
