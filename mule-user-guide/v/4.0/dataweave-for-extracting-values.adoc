...
Or you can use an expression on a Twitter connector to pick the userId to querry:
...
<set-payload value="#[variable['originalPayload']]" />
----
...
[source, xml, linenums]
----
<expression-transformer expression="#[($.value in  inboundAttachments.entrySet() if $.key ~= '(.*\\.txt|.*\\.xml)')]" />
----
...
If you're referencing an array or object, you can extract specific values from within the contents of this element. You can use any of the four types of selectors in DataWeave to navigate the data structure:
