...
This document describes the general configuration for connectors that work on Mule 4.
...
Connectors often require a configuration step whether they function as message sources or simply perform operations. The configuration can be referenced globally, using a global element that stores what is called the *connector configuration* or simply, *configuration*. The configuration is always specific to a connector type.
...
Some connectors can listen for events, producing a Mule message then pushing it into a Mule flow. These message sources are usually listening on external resources such as databases, Web clients, or email servers. As with the operation-based connector, you normally fill out fields in a connector configuration. However, in this case the configuration is used to set up to listen at an endpoint.

[NOTE]
====
You can think of a connector that acts as a *Message Source* in terms of endpoints. In this scenario, a connector triggers a new flow instance each time it receives a message.
...
The POP3 component is acting as the message source in the example flow:
...
== Email Connector as Message Source - Example

If a connector is not the first building block (i.e., the message source) in a flow, it is performing an operation. It may use a specific transport channel it supports (such as SMTP, FTP, or JDBC) to dispatch messages to targets outside the flow, which can range from file systems to email servers to Web clients and can also include other Mule flows.

In many cases, the connector completes a flow by dispatching a fully processed message to its final, external destination. However, they don't always mark the end of flow processing, because they can also exist in the middle of a flow, dispatching data to an external source, and also passing that (or some other data) to the next message processor in the flow.
