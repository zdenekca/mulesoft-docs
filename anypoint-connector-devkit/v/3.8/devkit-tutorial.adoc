...
Get the cookbook, which is stored on GitHub:

. Clone the link:https://github.com/mulesoft/mule-cookbook[https://github.com/mulesoft/mule-cookbook] repository into your local machine. For example, to clone to your desired development folder, run:
+
...
[[processor]]
=== Add an Operation with @Processor Annotation

Since we don't need any kind of authentication to consume the recently added recipes, `getRecentlyAdded` is an operation we can implement quickly to start learning how to build our connector.
