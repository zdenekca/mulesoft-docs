...
After learning some of the basic functionality in the link:/anypoint-studio/v/6/basic-studio-tutorial[Basic Studio Tutorial] and link:/getting-started/build-a-hello-world-application[How to Create a Mule App Tutorial], it's time to try your hand at enriching your application with an *Anypoint Connector*.

[NOTE]
link:/anypoint-studio/v/6/[Anypoint Studio] comes with a set of connectors that you can easily add to your Studio flow to connect your application. You can find all certified connectors in link:/getting-started/anypoint-exchange[Anypoint Exchange].
...
. Store the authentication, connection or other settings in a *properties file* link:/mule-user-guide/v/3.8/configuring-properties[Configuring Properties].
. Reference the properties in the link:/mule-user-guide/v/3.8/global-elements[global element] specific to your connector.
. Test the connection.
...
. To create a new *Twitter Application*, navigate to link:https://apps.twitter.com/[https://apps.twitter.com/].
...
Later, if you want to expose such an application publicly, you need to configure your application to link:/mule-user-guide/v/3.8/using-a-connector-to-access-an-oauth-api[authenticate via OAuth].
====
+
...
. Create a new Mule Project named `Connector Tutorial`. If you need more directions on launching Studio and creating a project, see link:/anypoint-studio/v/6/basic-studio-tutorial[Basic Studio Tutorial]. Press CMD + s (or CTRL + s in Windows) to save your project.
...
[cols="25a,75a"]
|===
|*link:/mule-user-guide/v/3.8/http-connector[HTTP Connector]* |Allows your Mule application to connect to web resources through the HTTP or HTTPS protocol. The HTTP connector receives an HTTP request.
...
|*link:/mule-user-guide/v/3.8/foreach[Foreach Scope]* |Processes each item in a POJO iteratively.
|*link:/mule-user-guide/v/3.8/logger-component-reference[Logger Component]* |Logs messages or activity based on the value of a Mule expression. In this application, logs each individual Tweet, displaying the Tweets in Studio's console.
|===
...
Read more about link:/mule-user-guide/v/3.8/global-elements[Global Elements in Mule].
...
INFO  2016-05-03 04:52:21,956 [[connector_tutorial].HTTP_Listener_Configuration.worker.01] org.mule.api.processor.LoggerMessageProcessor: RT @MuleDev: A tutorial by @PriyobrotoGhos2 on how to use #Java Arrays with Oracle database and #Mule! https://t.co/i7t4N1tVvN
...
There are a few ways to accomplish this task, one of which is to add the extra text to the Tweet inside a new variable. However, you can take a shortcut using the link:/mule-user-guide/v/3.8/set-payload-transformer-reference[set payload] transformer. Replace the content of the entire payload using an expression composed of multiple parts, such as the following:
...
* *NEXT STEP:* Try the link:/getting-started/mule-message[Mule Message Tutorial].
* link:https://www.mulesoft.com/exchange#!/?types=connector&sortBy=name[Install Connectors from Anypoint Exchange].
* Import link:/mule-user-guide/v/3.8/installing-connectors[additional connectors] into your instance of Anypoint Studio.
